...
namespace zCode.zField
{
    ...
    internal class GridField3dDouble : GridField3d<double>
    {
        ...
        protected sealed override double ValueAtLinear(Vec3d point)
        {
            ...
            double w = zMath.Fract(point.Z, out int k0);
            ...
            return zMath.Lerp(
                zMath.Lerp(
                    zMath.Lerp(vals[i0 + j0 + k0], vals[i1 + j0 + k0], u),
                    zMath.Lerp(vals[i0 + j1 + k0], vals[i1 + j1 + k0], u),
                    v),
                zMath.Lerp(
                    zMath.Lerp(vals[i0 + j0 + k1], vals[i1 + j0 + k1], u),
                    zMath.Lerp(vals[i0 + j1 + k1], vals[i1 + j1 + k1], u),
                    v),
                w);
        }
