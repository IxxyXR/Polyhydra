        ...
        Pass {
            CGPROGRAM
                ...
                v2f vert (appdata_t v) {
                    ...
                    float4 wN = float4(
                        (sin(1-v.normal.x * v.vertex.x * 6 * PI)+1)/2,
                        (sin(1-v.normal.y * v.vertex.y * 6 * PI)+1)/2,
                        (sin(1-v.normal.z * v.vertex.z * 6 * PI)+1)/2,
                        (cos(1-v.normal.z * v.vertex.z * 6 * PI)+1)/2
                    );	//Here I'm using the vertex position since this is for a sphere around the origin 
                    ...
                    //o.pos = UnityObjectToClipPos(float4(shC.rgb, 1.0f)); //Comment this in to see the shape of the spherical harmonic
